...
function ColorPaletteEditor({ theme, onThemeChange, floating = false, isOpen = false, onClose }) {
  ...
  if (floating) {
    // Original floating theme customizer (kept for backward compatibility)
    const [floatingIsOpen, setFloatingIsOpen] = useState(false);

    return (
      <>
        {/* Floating Theme Button */}
        <button
          onClick={() => setFloatingIsOpen(!floatingIsOpen)}
        ...
        {floatingIsOpen && (
          <>
            ...
            <div 
              ...
              onClick={() => setFloatingIsOpen(false)}
            />
  ...
  // Header-integrated theme customizer
  if (!isOpen) return null;

  return (
    <>
      {/* Backdrop with higher z-index to ensure it appears above everything */}
      <div 
        className="fixed inset-0 bg-black/20 backdrop-blur-sm z-[100]"
        onClick={onClose}
      />
      
      {/* Panel positioned below header with highest z-index */}
      <div 
        className="absolute top-full left-1/2 transform -translate-x-1/2 w-96 mt-2 rounded-2xl shadow-2xl z-[101] overflow-hidden"
        style={{
          background: theme.background,
          border: `1px solid ${theme.primary}20`
        }}
